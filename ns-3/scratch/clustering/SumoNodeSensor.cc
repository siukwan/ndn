...
double SumoNodeSensor::getX()
{
	...
	NS_ASSERT(m_sumodata->GetTrace(m_id,pos).x == pos.x);
...
double SumoNodeSensor::getY()
{
	...
	Ptr<Node> node = this->GetObject<Node>();
	uint32_t m_id = node->GetId();
	NS_ASSERT(m_sumodata->GetTrace(m_id,pos).y == pos.y);
...
std::pair<bool, double> SumoNodeSensor::getDistanceWith(const double& x,const double& y,const std::vector<std::string>& route)
{
	...
	{
		//From behind
		eit = edges.find(route.at(remoteIndex));
		...
		{
			eit = edges.find(route.at(i));
			NS_ASSERT_MSG(eit!=edges.end(),"No edge info for "<<route.at(i));
	...
	{
		//From front
		eit = edges.find(route.at(localIndex));
		NS_ASSERT_MSG(eit!=edges.end(),"No edge info for "<<remoteLane);
		//=======.=======|=========|========|===============.=============
